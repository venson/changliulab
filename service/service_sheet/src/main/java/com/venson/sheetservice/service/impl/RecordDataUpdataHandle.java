...
import com.venson.sheetservice.db.IRecordDataInsertHandle;
import com.venson.sheetservice.db.IRecordDataUpdataHandle;
...
import com.venson.sheetservice.utils.JfGridFileUtil;
...
public class RecordDataUpdataHandle extends BaseHandle implements IRecordDataUpdataHandle {
    ...
    public boolean updateCellDataListTxtValue(JSONObject query, String keyName, Integer position, Object v) {
        try{
            ...
            updateSql.append("update " + JfGridConfigModel.TABLENAME + " t set t.json_data=json_set(t.json_data,\"$.").append(keyName).append("\",");
            if(v instanceof JSON){
                updateSql.append("CAST('").append(((JSONObject) v).toJSONString()).append("' as JSON)");
            }else{
                updateSql.append("\"").append(v).append("\"");
            }
    ...
    public boolean updateCellDataListValue(JSONObject query, String keyName, String position, Object v) {
        try{
            ...
            if(v instanceof JSON){
                updateSql.append("CAST('"+((JSONObject) v).toJSONString()+"' as JSON)");
            }else{
                updateSql.append("\""+v+"\"");
            }
    ...
    public boolean rmJsonbDataForEmpty(JSONObject query, String word) {
        try{
            ...
            StringBuffer updateSql=new StringBuffer();
            updateSql.append("update "+JfGridConfigModel.TABLENAME+" t set "+ sb);
            updateSql.append(" where 1=1 "+condition);
            log.info("updateSql:"+ updateSql);
            luckySheetJdbcTemplate.update(updateSql.toString(),arr.toArray());
            return true;
    ...
    public boolean updateDataStatus(GridRecordDataModel model) {
        try{
            ...
            luckySheetJdbcTemplate.update(sql1, model.getList_id(),model.getBlock_id());

            String sql2="update "+JfGridConfigModel.TABLENAME+" set status=1  where  list_id=? and index=? and block_id=?";
            log.info("updateSql2:"+sql2);
            luckySheetJdbcTemplate.update(sql2, model.getList_id(),model.getIndex(),model.getBlock_id());
            return true;
        }catch (Exception e){
            log.error(e.getMessage());
            throw new RuntimeException(e.getMessage());
    ...
    public boolean updateDataMsgHide(GridRecordDataModel model, Integer hide, String index1, String index2) {
        try{
            ...
            log.info("updateSql1:"+sql1);
            luckySheetJdbcTemplate.update(sql1, model.getList_id(),index1,JfGridConfigModel.FirstBlockID);
            String sql2="update "+JfGridConfigModel.TABLENAME+" set status=1  where  list_id=? and `index`=? and block_id=?";
            log.info("updateSql2:"+sql2);
            luckySheetJdbcTemplate.update(sql2, model.getList_id(),index2,model.getBlock_id());
            return true;
        }catch (Exception e){
            log.error(e.getMessage());
            throw new RuntimeException(e.getMessage());
    ...
    public boolean updateDataMsgNoHide(GridRecordDataModel model, Integer hide, String index) {
        try{
            ...
            log.info("updateSql2:"+sql2);
            luckySheetJdbcTemplate.update(sql2, model.getList_id(),index,model.getBlock_id());
            return true;
        }catch (Exception e){
            log.error(e.getMessage());
            throw new RuntimeException(e.getMessage());
    ...
    public boolean batchUpdateCellDataValue(List<String> block_ids, List<GridRecordDataModel> models) {
        try{
            ...
            String delsql= "DELETE from "+
                    JfGridConfigModel.TABLENAME+
                    " t where t.list_id=? and t.index=? and t.block_id in ("
                    +id+
                    ")";
            luckySheetJdbcTemplate.update(delsql, models.get(0).getList_id(),models.get(0).getIndex());
            ...
            for(GridRecordDataModel b : models){
                ...
                Object[] params= objectList.toArray(new Object[objectList.size()]);
                batch.add(params);
            }
            log.info("sqls:{}",sql);
